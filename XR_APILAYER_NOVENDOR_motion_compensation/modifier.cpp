...
namespace modifier
{
    ...
    void TrackerModifier::Apply(XrPosef& target, const XrPosef& reference) const
    {
        ...
        if (m_ApplyRotation)
        {
            ...
            //TraceLoggingWriteTagged(local, "TrackerModifier::Apply", TLArg(xr::ToString(angles).c_str(), "Original Angles"));

            angles = {angles.x * m_Pitch, angles.y * m_Yaw, angles.z * m_Roll};
            //TraceLoggingWriteTagged(local, "TrackerModifier::Apply", TLArg(xr::ToString(angles).c_str(), "Modified Angles"));
        ...
        if (m_ApplyTranslation)
        {
            ...
            //TraceLoggingWriteTagged(local, "TrackerModifier::Apply", TLArg(xr::ToString(translation).c_str(), "Original Translation"));

            translation = XrVector3f{translation.x * m_Sway, translation.y * m_Heave, translation.z * m_Surge};
            //TraceLoggingWriteTagged(local, "TrackerModifier::Apply", TLArg(xr::ToString(translation).c_str(), "Modified Translation"));
    ...
    void HmdModifier::Apply(XrPosef& target, const XrPosef& reference) const
    {
        ...
        if (m_ApplyTranslation)
        {
            ...
            //TraceLoggingWriteTagged(local, "HmdModifier::Apply", TLArg(xr::ToString(translation).c_str(), "Original Translation"));

            translation = XrVector3f{translation.x * m_Sway, translation.y * m_Heave, translation.z * m_Surge};
            //TraceLoggingWriteTagged(local, "HmdModifier::Apply", TLArg(xr::ToString(translation).c_str(), "Modified Translation"));
