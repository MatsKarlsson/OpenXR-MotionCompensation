...
namespace tracker
{
    ...
    bool ControllerBase::GetPoseDelta(XrPosef& poseDelta, XrSession session, XrTime time)
    {
        ...
        if (time == m_LastPoseTime)
        {
            ...
            //TraceLoggingWriteStop(local, "ControllerBase::GetPoseDelta", TLArg(xr::ToString(m_LastPoseDelta).c_str(), "LastDelta"));
        ...
        if (XrPosef curPose{Pose::Identity()}; GetPose(curPose, session, time))
        {
            ...
            //TraceLoggingWriteStop(local, "ControllerBase::GetPoseDelta", TLArg(true, "Success"), TLArg(xr::ToString(poseDelta).c_str(), "Delta"));
            return true;
        }
    ...
    {
        //TraceLocalActivity(local);
        //TraceLoggingWriteStart(local, "ControllerBase::ResetReferencePose", TLXArg(session, "Session"), TLArg(time, "Time"));
    ...
    {
        //TraceLocalActivity(local);
        //TraceLoggingWriteStart(local, "ControllerBase::GetControllerPose", TLXArg(session, "Session"), TLArg(time, "Time"));
        ...
        if (auto* layer = reinterpret_cast<OpenXrLayer*>(GetInstance()))
        {
            ...
            if (!layer->m_XrSyncCalled.load())
            {
                ...
                //TraceLoggingWriteTagged(local, "ControllerBase::GetControllerPose", TLXArg(layer->m_ActionSet, "xrSyncActions"));
            ...
            }
            {
                ...
                //TraceLoggingWriteTagged(local, "ControllerBase::GetControllerPose", TLXArg(layer->m_PoseAction, "xrGetActionStatePose"));
    ...
    XrPosef TrackerBase::GetReferencePose() const
    {
        ...
        //TraceLoggingWriteStop(local, "TrackerBase::GetReferencePose");
    ...
    bool TrackerBase::CalibrateForward(XrSession session, XrTime time, float yawOffset)
    {
        ...
        //TraceLoggingWriteStop(local, "TrackerBase::CalibrateForward", TLArg(true, "Success"));
    ...
    {
        //TraceLocalActivity(local);
        //TraceLoggingWriteStart(local, "OpenXrTracker::ReadSource", TLArg(time, "Time"));
    ...
    bool OpenXrTracker::GetPose(XrPosef& trackerPose, XrSession session, XrTime time)
    {
        ...
        else
        {
            ...
            //TraceLoggingWriteTagged(local, "OpenXrTracker::GetPose", TLArg(true, "Sampled"));
        }
    ...
    bool VirtualTracker::ResetReferencePose(const XrSession session, const XrTime time)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                //TraceLoggingWriteTagged(local, "VirtualTracker::ResetReferencePose", TLArg(xr::ToString(offset).c_str(), "Offset"));
        ...
        //TraceLoggingWriteStop(local, "VirtualTracker::ResetReferencePose", TLArg(success, "Success"));
    ...
    bool VirtualTracker::GetPose(XrPosef& trackerPose, const XrSession session, const XrTime time)
    {
        ...
        //TraceLoggingWriteTagged(local, "VirtualTracker::GetPose", TLArg(xr::ToString(dof).c_str(), "Data"));
        ...
        if (0 != m_PitchConstant)
        {
            ...
            //TraceLoggingWriteTagged(local, "VirtualTracker::GetPose");
                ...
                if (XR_SUCCEEDED(result))
                {
                    if (Pose::IsPoseValid(hmdLocation.locationFlags))
                    {
                        ...
                        if (m_Mmf.Read(&data, sizeof(data), time))
                        {
                            ...
                            //TraceLoggingWriteTagged(local, "YawTracker::ResetReferencePose");
                                                    //TLXArg(this->m_OffsetForward, "OffsetForward"),
                                                    //TLXArg(this->m_OffsetDown, "OffsetDown"));
                        }
                        else
                        {
    ...
    void CorManipulator::GetButtonState(XrSession session, bool& moveButton, bool& positionButton)
    {
         ...
         {
            // sync actions
            //TraceLoggingWriteTagged(local, "CorManipulator::GetButtonState", TLXArg(layer->m_ActionSet, "xrSyncActions"));
            constexpr XrActionsSyncInfo syncInfo{XR_TYPE_ACTIONS_SYNC_INFO, nullptr, 0, nullptr};
         ...
         }
         {
            ...
            //TraceLoggingWriteTagged(local, "CorManipulator::GetButtonState", TLXArg(layer->m_MoveAction, "xrGetActionStateBoolean"));
                ...
                XR_FAILED(result))
            {
                ...
                //TraceLoggingWriteStop(local, "CorManipulator::GetButtonState", TLArg(false, "xrGetActionStateBooleanSuccess_Move"));
                return;
            }
            ...
            //TraceLoggingWriteTagged(local, "CorManipulator::GetButtonState", TLXArg(layer->m_PositionAction, "xrGetActionStateBoolean"));
            ...
            {
                ErrorLog("%s: xrGetActionStateBoolean failed: %s", __FUNCTION__, xr::ToCString(result));
                //TraceLoggingWriteStop(local, "CorManipulator::GetButtonState", TLArg(false, "xrGetActionStateBooleanSuccess_Position"));
                return;
            }
            if (positionButtonState.isActive == XR_TRUE)
            {
                positionButton = XR_TRUE == positionButtonState.currentState;
                //TraceLoggingWriteTagged(local, "CorManipulator::GetButtonState", TLArg(positionButton, "PositionButtonState"));
            }
         }
    ...
    {
         //TraceLocalActivity(local);
        //TraceLoggingWriteStart(local, "CorManipulator::ApplyPosition");
    ...
    {
        //TraceLocalActivity(local);
        //TraceLoggingWriteStart(local, "CorManipulator::ApplyTranslation");
    ...
    {
         //TraceLocalActivity(local);
         //TraceLoggingWriteStart(local, "CorManipulator::ApplyRotation", TLArg(xr::ToString(poseDelta).c_str(), "Delta"));
